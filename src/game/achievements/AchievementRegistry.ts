...
const progressionAchievements: AchievementDefinition[] = [
  ...
  achievement('progress_distance_250', loc('Hublot de Brume', 'Mist Porthole'), loc('Parcourir 250 m dans une run.', 'Travel 250m in a single run.'), 'progression', 'common', 250, { rewardId: reward('background', 2) }),
  achievement('progress_distance_500', loc('Houle du Large', 'Open-Sea Swell'), loc('Parcourir 500 m dans une run.', 'Travel 500m in a single run.'), 'progression', 'uncommon', 500, { rewardId: reward('background', 3) }),
  ...
  achievement('progress_distance_1500', loc('Traverse de Verre', 'Glass Traverse'), loc('Parcourir 1 500 m dans une run.', 'Travel 1,500m in a single run.'), 'progression', 'rare', 1500, { rewardId: reward('motif', 2) }),
...
const shardAchievements: AchievementDefinition[] = [
  ...
  achievement('shards_land_10', loc('Mousse des Fragments', 'Fragment Deckhand'), loc('Atterrir 10 fois.', 'Land 10 times.'), 'shards', 'common', 10, { rewardId: reward('background', 4) }),
  achievement('shards_land_25', loc('Gabier des Éclats', 'Shard Lookout'), loc('Atterrir 25 fois.', 'Land 25 times.'), 'shards', 'uncommon', 25, { rewardId: reward('face', 4) }),
  achievement('shards_land_50', loc('Pilote des Reflets', 'Pilot of Reflections'), loc('Atterrir 50 fois.', 'Land 50 times.'), 'shards', 'uncommon', 50, { rewardId: reward('background', 5) }),
  achievement('shards_land_100', loc('Cap sur les Verrières', 'Course for the Panes'), loc('Atterrir 100 fois.', 'Land 100 times.'), 'shards', 'rare', 100, { hidden: true }),
  ...
  achievement('shards_land_300', loc('Capitaine du Verre Fendu', 'Captain of Broken Glass'), loc('Atterrir 300 fois.', 'Land 300 times.'), 'shards', 'legendary', 300, { hidden: true }),
  achievement('shards_chain_8', loc('Chapelet d’Éclats', 'String of Shards'), loc('Enchaîner 8 atterrissages dans une run.', 'Chain 8 landings in a run.'), 'shards', 'uncommon', 8, { rewardId: reward('face', 5) }),
  achievement('shards_chain_12', loc('Nœud de Reflets', 'Reflection Knot'), loc('Enchaîner 12 atterrissages dans une run.', 'Chain 12 landings in a run.'), 'shards', 'uncommon', 12, { hidden: true }),
  achievement('shards_chain_20', loc('Route de Verre', 'Glass Route'), loc('Enchaîner 20 atterrissages dans une run.', 'Chain 20 landings in a run.'), 'shards', 'rare', 20, { rewardId: reward('motif', 3) }),
  achievement('shards_chain_30', loc('Convoi des Brumes', 'Fog Convoy'), loc('Enchaîner 30 atterrissages dans une run.', 'Chain 30 landings in a run.'), 'shards', 'epic', 30, { hidden: true }),
  achievement('shards_chain_40', loc('Couronne de Shards', 'Crown of Shards'), loc('Enchaîner 40 atterrissages dans une run.', 'Chain 40 landings in a run.'), 'shards', 'legendary', 40, { hidden: true }),
  achievement('shards_milestone_1', loc('Balise de Marée', 'Tide Beacon'), loc('Atteindre une shard milestone.', 'Reach a milestone shard.'), 'shards', 'uncommon', 1, { rewardId: reward('face', 6) }),
  ...
  achievement('shards_triangular_5', loc('Dents de Reflets', 'Mirror Fangs'), loc('Dompter 5 shards triangulaires.', 'Master 5 triangular shards.'), 'shards', 'epic', 5, { hidden: true, rewardId: reward('eyes', 4) }),
...
const skillAchievements: AchievementDefinition[] = [
  ...
  achievement('skill_twist_15', loc('Danse du Macaque', 'Macaque Dance'), loc('Réaliser 15 twists.', 'Perform 15 twists.'), 'skill', 'common', 15, { rewardId: reward('background', 6) }),
  ...
  achievement('skill_twist_50', loc('Tourbillon Primate', 'Primate Whirl'), loc('Réaliser 50 twists.', 'Perform 50 twists.'), 'skill', 'rare', 50, { rewardId: reward('motif', 4) }),
  ...
  achievement('skill_perfect_10', loc('Verre Bien Taillé', 'Sharpened Glass'), loc('Réussir 10 atterrissages perfect.', 'Land 10 perfects.'), 'skill', 'uncommon', 10, { rewardId: reward('face', 7) }),
  achievement('skill_perfect_25', loc('Éclat Chirurgical', 'Surgical Shard'), loc('Réussir 25 atterrissages perfect.', 'Land 25 perfects.'), 'skill', 'rare', 25, { rewardId: reward('eyes', 5) }),
  ...
  achievement('skill_perfect_streak_3', loc('Coupe Miroir', 'Mirror Cut'), loc('Réaliser 3 perfects d’affilée.', 'Chain 3 perfects in a row.'), 'skill', 'epic', 3, { hidden: true, rewardId: reward('eyes', 6) }),
...
const momentumAchievements: AchievementDefinition[] = [
  achievement('momentum_reach_80', loc('Brise de Pont', 'Deck Breeze'), loc('Atteindre 80 % de momentum.', 'Reach 80% momentum.'), 'momentum', 'uncommon', 1, { hidden: true }),
  ...
  achievement('momentum_hold_max_5', loc('Marée Tendue', 'Tension Tide'), loc('Garder le momentum max 5 secondes.', 'Hold max momentum for 5 seconds.'), 'momentum', 'rare', 5, { rewardId: reward('motif', 5) }),
  ...
  achievement('momentum_hold_high_45', loc('Courant des Singes', 'Ape Current'), loc('Rester en momentum élevé 45 secondes cumulées.', 'Stay at high momentum for 45 cumulative seconds.'), 'momentum', 'rare', 45, { rewardId: reward('eyes', 7) }),
...
const combatAchievements: AchievementDefinition[] = [
  ...
  achievement('combat_kill_10', loc('Croc en Recul', 'Backstep Fang'), loc('Éliminer 10 ennemis.', 'Eliminate 10 enemies.'), 'combat', 'common', 10, { rewardId: reward('background', 7) }),
  achievement('combat_kill_25', loc('Flibustier des Ombres', 'Shadow Freebooter'), loc('Éliminer 25 ennemis.', 'Eliminate 25 enemies.'), 'combat', 'uncommon', 25, { hidden: true }),
  ...
  achievement('combat_back_run_10', loc('Main de Traverse', 'Crosswind Hand'), loc('Tuer 10 ennemis par l’arrière dans une même run.', 'Kill 10 enemies from behind in a single run.'), 'combat', 'rare', 10, { rewardId: reward('barbe', 3) }),
  ...
  achievement('combat_shield_save_10', loc('Garde de Quart', 'Watch Guard'), loc('Survivre 10 fois grâce au Bouclier.', 'Survive 10 times thanks to the Shield.'), 'combat', 'rare', 10, { rewardId: reward('barbe', 4) }),
...
const economyAchievements: AchievementDefinition[] = [
  ...
  achievement('economy_coins_100', loc('Bourse Fendue', 'Split Purse'), loc('Ramasser 100 pièces.', 'Collect 100 coins.'), 'economy', 'uncommon', 100, { rewardId: reward('background', 8) }),
  achievement('economy_coins_300', loc('Coffre de Verre', 'Glass Chest'), loc('Ramasser 300 pièces.', 'Collect 300 coins.'), 'economy', 'rare', 300, { hidden: true }),
  achievement('economy_coins_500', loc('Trésor de Cale', 'Hold Treasure'), loc('Ramasser 500 pièces.', 'Collect 500 coins.'), 'economy', 'rare', 500, { rewardId: reward('barbe', 5) }),
  ...
  achievement('economy_buy_15', loc('Atelier de Bord', 'Deck Workshop'), loc('Acheter 15 items en boutique.', 'Buy 15 shop items.'), 'economy', 'rare', 15, { hidden: true }),
  ...
  achievement('modules_rare_1', loc('Première Trouvaille Rare', 'First Rare Find'), loc('Obtenir un module rare.', 'Collect a rare module.'), 'modules', 'uncommon', 1, { rewardId: reward('background', 9) }),
  ...
  achievement('modules_legendary_1', loc('Première Relique', 'First Relic'), loc('Obtenir un module légendaire.', 'Collect a legendary module.'), 'modules', 'epic', 1, { hidden: true, rewardId: reward('eyes', 10) }),
...
const moduleAchievements: AchievementDefinition[] = [
  ...
  achievement('modules_unique_6', loc('Sac du Gabier', 'Lookout Satchel'), loc('Obtenir 6 modules différents.', 'Collect 6 different modules.'), 'modules', 'uncommon', 6, { rewardId: reward('face', 8) }),
  achievement('modules_unique_9', loc('Pont Bien Garni', 'Loaded Deck'), loc('Obtenir 9 modules différents.', 'Collect 9 different modules.'), 'modules', 'rare', 9, { rewardId: reward('eyes', 8) }),
  achievement('modules_unique_12', loc('Collection du Capitaine', 'Captain’s Collection'), loc('Obtenir les 12 modules principaux au moins une fois.', 'Collect all 12 main modules at least once.'), 'modules', 'epic', 12, { rewardId: reward('eyes', 9) }),
  achievement('modules_plane_1', loc('Première Toile', 'First Sailcloth'), loc('Obtenir le Planeur.', 'Collect the Glider.'), 'modules', 'common', 1, { hidden: true }),
  achievement('modules_shield_1', loc('Première Carapace', 'First Carapace'), loc('Obtenir le Bouclier.', 'Collect the Shield.'), 'modules', 'common', 1, { hidden: true }),
  achievement('modules_souffleur_1', loc('Premier Souffle', 'First Gust'), loc('Obtenir le Souffleur.', 'Collect the Blower.'), 'modules', 'common', 1, { hidden: true }),
  achievement('modules_propulseur_1', loc('Premier Réacteur', 'First Thruster'), loc('Obtenir le Réacteur.', 'Collect the Reactor.'), 'modules', 'common', 1, { hidden: true }),
  achievement('modules_reacteur_back_1', loc('Première Poupe Chaude', 'First Warm Stern'), loc('Obtenir le Réacteur arrière.', 'Collect the Rear Reactor.'), 'modules', 'common', 1, { hidden: true }),
  achievement('modules_reacteur_front_1', loc('Première Proue Chaude', 'First Warm Prow'), loc('Obtenir le Réacteur avant.', 'Collect the Front Reactor.'), 'modules', 'common', 1, { hidden: true }),
  achievement('modules_wings_10', loc('Battement d’Écume', 'Foambeat'), loc('Utiliser les Ailes 10 fois.', 'Use Wings 10 times.'), 'modules', 'uncommon', 10, { rewardId: reward('face', 9) }),
  achievement('modules_grappin_10', loc('Accroche-Houle', 'Swell Hook'), loc('Réussir 10 grappins.', 'Land 10 successful grapples.'), 'modules', 'rare', 10, { rewardId: reward('barbe', 6) }),
