...
const progressionAchievements: AchievementDefinition[] = [
  ...
  achievement('progress_distance_250', loc('Tu peux faire mieux', 'You can do better'), loc('Parcourir 250 m dans une run.', 'Travel 250m in a single run.'), 'progression', 'common', 250, { rewardId: reward('background', 2) }),
  achievement('progress_distance_500', loc('Tu prends la distance', 'You’re getting somewhere'), loc('Parcourir 500 m dans une run.', 'Travel 500m in a single run.'), 'progression', 'uncommon', 500, { rewardId: reward('background', 3) }),
  ...
  achievement('progress_distance_1500', loc('Toujours pas coulé', 'Still not sunk'), loc('Parcourir 1 500 m dans une run.', 'Travel 1,500m in a single run.'), 'progression', 'rare', 1500, { rewardId: reward('motif', 2) }),
  ...
  achievement('progress_distance_3000', loc('Le rivage te cherche encore', 'The shore lost you'), loc('Parcourir 3 000 m dans une run.', 'Travel 3,000m in a single run.'), 'progression', 'epic', 3000, { rewardId: reward('eyes', 2) }),
  achievement('progress_distance_4000', loc('Même les mouettes ont lâché', 'Even the gulls gave up'), loc('Parcourir 4 000 m dans une run.', 'Travel 4,000m in a single run.'), 'progression', 'epic', 4000, { hidden: true, rewardId: reward('eyes', 3) }),
  ...
  achievement('progress_survive_120', loc('Pas très terrestre tout ça', 'Not very ground-based'), loc('Cumuler 120 secondes réelles en l’air dans une run.', 'Accumulate 120 real airtime seconds in a run.'), 'progression', 'common', 120, { rewardId: reward('face', 2) }),
  achievement('progress_survive_180', loc('Le sol te manque pas ?', 'Don’t you miss the ground?'), loc('Cumuler 180 secondes réelles en l’air dans une run.', 'Accumulate 180 real airtime seconds in a run.'), 'progression', 'uncommon', 180, { rewardId: reward('face', 3) }),
  achievement('progress_survive_300', loc('Pilote sans permis', 'Unlicensed pilot'), loc('Cumuler 300 secondes réelles en l’air dans une run.', 'Accumulate 300 real airtime seconds in a run.'), 'progression', 'rare', 300, { rewardId: reward('barbe', 2) }),
...
const shardAchievements: AchievementDefinition[] = [
  ...
  achievement('shards_chain_20', loc('Là c’est propre', 'Now that’s clean'), loc('Enchaîner 2 atterrissages Perfect.', 'Chain 2 Perfect landings.'), 'shards', 'rare', 2, { rewardId: reward('motif', 3) }),
...
const skillAchievements: AchievementDefinition[] = [
  ...
  achievement('skill_twist_15', loc('Tu tournes bien toi', 'You spin nicely'), loc('Réaliser 15 twists.', 'Perform 15 twists.'), 'skill', 'common', 15, { rewardId: reward('background', 6) }),
  ...
  achievement('skill_twist_50', loc('T’as mangé une toupie ?', 'Did you eat a spinning top?'), loc('Réaliser 50 twists.', 'Perform 50 twists.'), 'skill', 'rare', 50, { rewardId: reward('motif', 4) }),
  ...
  achievement('skill_perfect_10', loc('Là, tu pinaille', 'Now you’re nitpicking'), loc('Réussir 10 atterrissages perfect.', 'Land 10 perfects.'), 'skill', 'uncommon', 10, { rewardId: reward('face', 7) }),
  achievement('skill_perfect_25', loc('Tu coupes les angles au rasoir', 'Cutting corners with a scalpel'), loc('Réussir 25 atterrissages perfect.', 'Land 25 perfects.'), 'skill', 'rare', 25, { rewardId: reward('eyes', 5) }),
  ...
  achievement('skill_perfect_streak_3', loc('Continue, respire pas', 'Keep going, don’t blink'), loc('Réaliser 3 perfects d’affilée.', 'Chain 3 perfects in a row.'), 'skill', 'epic', 3, { hidden: true, rewardId: reward('eyes', 6) }),
...
const momentumAchievements: AchievementDefinition[] = [
  ...
  achievement('momentum_hold_max_5', loc('Tiens bon la barre', 'Hold that speed'), loc('Garder le momentum max 5 secondes.', 'Hold max momentum for 5 seconds.'), 'momentum', 'rare', 5, { rewardId: reward('motif', 5) }),
  ...
  achievement('momentum_hold_high_45', loc('Ça file sans clignoter', 'Now that’s sustained speed'), loc('Rester en momentum élevé 45 secondes cumulées.', 'Stay at high momentum for 45 cumulative seconds.'), 'momentum', 'rare', 45, { rewardId: reward('eyes', 7) }),
...
const combatAchievements: AchievementDefinition[] = [
  ...
  achievement('combat_kill_10', loc('Ça tape déjà mieux', 'You’re getting the hang of violence'), loc('Éliminer 10 ennemis.', 'Eliminate 10 enemies.'), 'combat', 'common', 10, { rewardId: reward('background', 7) }),
  ...
  achievement('combat_back_run_10', loc('Par derrière, évidemment', 'From behind, obviously'), loc('Tuer 10 ennemis par l’arrière dans une même run.', 'Kill 10 enemies from behind in a single run.'), 'combat', 'rare', 10, { rewardId: reward('barbe', 3) }),
  ...
  achievement('combat_shield_save_10', loc('Sans ça, t’étais fini', 'That shield is carrying you'), loc('Survivre 10 fois grâce au Bouclier.', 'Survive 10 times thanks to the Shield.'), 'combat', 'rare', 10, { rewardId: reward('barbe', 4) }),
...
const moduleAchievements: AchievementDefinition[] = [
  ...
  achievement('modules_unique_6', loc('Tu commences un vrai build', 'That’s starting to look like a build'), loc('Obtenir 6 modules différents.', 'Collect 6 different modules.'), 'modules', 'uncommon', 6, { rewardId: reward('face', 8) }),
  achievement('modules_unique_9', loc('T’as pris tout le rayon', 'You bought half the shop'), loc('Obtenir 9 modules différents.', 'Collect 9 different modules.'), 'modules', 'rare', 9, { rewardId: reward('eyes', 8) }),
  achievement('modules_unique_12', loc('Plus d’excuse maintenant', 'No excuses now'), loc('Obtenir les 12 modules principaux au moins une fois.', 'Collect all 12 main modules at least once.'), 'modules', 'epic', 12, { rewardId: reward('eyes', 9) }),
  ...
  achievement('modules_wings_10', loc('Bats des ailes, pas des records', 'Flap first, ask questions later'), loc('Utiliser les Ailes 10 fois.', 'Use Wings 10 times.'), 'modules', 'uncommon', 10, { rewardId: reward('face', 9) }),
  achievement('modules_grappin_10', loc('Accroche-toi bien', 'Hang on tight'), loc('Réussir 10 grappins.', 'Land 10 successful grapples.'), 'modules', 'rare', 10, { rewardId: reward('barbe', 6) }),
  ...
  achievement('modules_wrapper_5', loc('Hop, plus là', 'Boop, gone'), loc('Utiliser le Téléporteur 5 fois.', 'Use the Teleporter 5 times.'), 'modules', 'rare', 5, { rewardId: reward('eyes', 11) }),
